...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ[
    'DB_PATH']  # 'sqlite:///../data.db' # sqlite:////root/IslesAssault/data.db
...
def shopveh(id):
    ...
    if request.method == 'GET':
        ...
        if (not acc.money < curItem.cost) and Account_Item.query.filter_by(accID=acc.id,
                                                                           itemID=curItem.id).first() is None:
...
def account():
    ...
    return render_template('user.html', lvlitems=lvls, ava=img, maxlvl=maxLevel + 1,
                           ...
                           time=(str(acc.playtime // 3600) + 'h') * int(acc.playtime // 3600 != 0) + str(
                               acc.playtime % 3600 // 60) + 'm',
                           KD=str(int(acc.kills / (acc.deaths + int(acc.deaths == 0)) * 100) / 100) * int(
                               acc.deaths != 0) + 'INVINCIBLE' * int(
                               acc.deaths == 0 and acc.kills > 0) + 'SIMPLETON' * int(
                               acc.deaths == 0 and acc.kills == 0))
...
def index():
    ...
    if logged and Account.query.filter_by(nickname=session['name']).first():
        ...
        servers = []
        if isDEV:
            servers = [['local 8001', 'wss://localhost:8001'], ['local 8002', 'wss://localhost:8002'],
                       ['local 8003', 'wss://localhost:8003']]
        else:
...
def about():
    return render_template('about.html', content=re.sub(':(\\S*):|\\[!(\\S*)\\]', ' ', markdown.markdown(
        requests.get('https://raw.githubusercontent.com/BRO-Fedka/IslesAssault/master/README.md').text)))
...
if __name__ == "__main__":
    app.run(host=os.environ['HOST'], port=int(os.environ['PORT']), debug=bool(isDEV))  # 26.223.93.1
