...
class GroundResistance(Module):
    ...
    def update_module(self,vehicle):
        ...
        # r_vec = (math.cos(deg) * COF_GROUND_RESISTANCE * self.body.velocity.length ** 2,
        #          math.sin(deg) * COF_GROUND_RESISTANCE * self.body.velocity.length ** 2)
        # l_r_vec = COF_GROUND_RESISTANCE * self.body.velocity.length ** 2
        # area = list(self.body.shapes)[0].area

        surface_friction = COF_GROUND_RESISTANCE * self.body.velocity.length ** 2
        # print(surface_friction)
        # print('Gin',surface_friction)
        # print(self.body.mass)
        # F*(t2-t1) = m*V
        leave = False
        if surface_friction > self.body.velocity.length * self.body.mass * TPS:
            surface_friction = self.body.velocity.length * self.body.mass * TPS * 0.5
            leave = True
        ...
        self.body.apply_force_at_local_point(
            (0, -self.body.angular_velocity / self.max_rotation_speed * self.body.moment), (1, 0))
        ...
        #         self.poly_n[coord][0] ** 2 + self.poly_n[coord][1] ** 2)
        #     m = COF_GROUND_RESISTANCE * vec_from_cntr_grav_l ** 2 * self.body.angular_velocity * abs(
