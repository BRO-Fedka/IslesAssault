...
class WaterResistance(Module):
    ...
    def __init__(self, poly_shape, poly_shape_n, resistance_cof=1, max_speed=0.1):
    ...
    def update_module_return_hp(self, body: Body, input: PlayerInputData) -> int:
        ...
        area = list(body.shapes)[0].area
        surface_friction = COF_WATER_SURFACE_FRICTION * body.velocity.length ** 2 * area
        body.apply_force_at_local_point((math.cos(deg) * surface_friction, math.sin(deg) * surface_friction),
        ...
        for coord in range(0, len(self.poly_shape)):
            ...
            if cos < 0:
                body.apply_force_at_local_point((self.poly_shape_n[coord][
                                                     0] / n_len * cos * l_r_vec * l * self.resistance_cof,
                                                 self.poly_shape_n[coord][
                                                     1] / n_len * cos * l_r_vec * l * self.resistance_cof), point)
            vec_from_cntr_grav = (point[0] - body.center_of_gravity.x, point[1] - body.center_of_gravity.y)
            ...
            cos = rotated_vec_from_cntr_grav[0] * self.poly_shape_n[coord][0] + rotated_vec_from_cntr_grav[1] * \
                ...
                self.poly_shape_n[coord][0] ** 2 + self.poly_shape_n[coord][1] ** 2)
            m = COF_WATER_RESISTANCE * vec_from_cntr_grav_l ** 2 * body.angular_velocity * abs(body.angular_velocity)
            if cos < 0:
        ...
        if body.velocity.length > self.max_speed:
            body.velocity = (body.velocity.x / body.velocity.length * self.max_speed,
                             body.velocity.y / body.velocity.length * self.max_speed)
        if body.velocity.length == 0 or abs(body.angular_velocity) < 0.05 * (
                body.velocity.length * 1 / self.max_speed) ** 5:
